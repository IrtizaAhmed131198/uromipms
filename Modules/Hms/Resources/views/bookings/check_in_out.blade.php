<div class="modal-dialog modal-xl" role="document">
        ...
        <div class="col-md-3">
                @if(empty($transaction->check_in))
                    <div class="form-group">
                        ...
                        {!! Form::text('in_out_date_time', @format_datetime($transaction->hms_booking_arrival_date_time), [
                ...
                @elseif (!empty($transaction->check_in) && empty($transaction->check_out))
                    <div class="form-group">
                        ...
                        {!! Form::text('in_out_date_time', @format_datetime($transaction->hms_booking_departure_date_time), [
                            'class' => 'form-control date_picker',
                            'required',
                            'readonly',
                        ]) !!}
    ...
    <div class="modal-footer" style="display:block;">
        ...
        @elseif (!empty($transaction->check_in) && empty($transaction->check_out))
            @php $due = $transaction->final_total - ($transaction->total_paid ?? 0); @endphp
            @if($due > 0)
                <div class="alert alert-warning" style="margin-bottom:12px;">
                    <strong>Outstanding Balance: @format_currency($due)</strong>
                </div>
                <div class="row" style="margin-bottom:12px;">
                    <div class="col-md-4">
                        <label>Payment Method</label>
                        <select name="payment_method" class="form-control" id="checkout_payment_method">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="payment_amount" id="checkout_payment_amount"
            ...
            @endif
            <div style="text-align:right;">
                <button type="submit" id="btn_checkout" class="tw-dw-btn tw-dw-btn-success tw-text-white">
                    @lang('hms::lang.confirm_check_out')
                </button>
                <button type="button" class="tw-dw-btn tw-dw-btn-neutral tw-text-white" data-dismiss="modal">@lang('messages.close')</button>
            </div>
            @if($due > 0)
            ...
            (function(){
                ...
                function checkAmount(){
                    var amt = parseFloat(document.getElementById('checkout_payment_amount').value) || 0;
                    document.getElementById('btn_checkout').disabled = (amt < due);
                }
                var inp = document.getElementById('checkout_payment_amount');
                if(inp){ inp.addEventListener('input', checkAmount); checkAmount(); }
            })();
            </script>
            @endif
        @endif
