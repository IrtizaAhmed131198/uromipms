...
class HmsRoom extends Model
{
    ...
    public function roomType()
    {
        return $this->belongsTo(
            \Modules\Hms\Entities\HmsRoomType::class,
            'hms_room_type_id'
        );
    }
    ...
    public static function non_booking_rooms($type_id, $arrival_date_time, $departure_date_time, $existing_rooms, $adate, $ddate)
    {
        $non_booking_rooms = HmsRoom::leftJoin('hms_booking_lines', function ($join) use ($arrival_date_time, $departure_date_time) {
            $join
                ->on('hms_rooms.id', '=', 'hms_booking_lines.hms_room_id')
                ...
                ->where(function ($query) use ($arrival_date_time, $departure_date_time) {
                    $query
                        ...
                        ->where(function ($q) use ($arrival_date_time, $departure_date_time) {
                            $q->whereRaw('transactions.hms_booking_arrival_date_time <= ? AND transactions.hms_booking_departure_date_time >= ?', [$departure_date_time, $arrival_date_time])
                                ...
                                ->orWhere(function ($q2) use ($arrival_date_time, $departure_date_time) {
                                    $q2->whereRaw('transactions.hms_booking_arrival_date_time <= ? AND transactions.hms_booking_departure_date_time >= ?', [$arrival_date_time, $departure_date_time]);
                                });
                        });
                })
            ...
            ->leftJoin('hms_room_unavailables', function ($join) use ($adate, $ddate) {
                $join
                    ->on('hms_rooms.id', '=', 'hms_room_unavailables.hms_rooms_id')
                    ->where(function ($query) use ($adate, $ddate) {
                        $query
                            ->whereDate('hms_room_unavailables.date_to', '>=', $adate)
