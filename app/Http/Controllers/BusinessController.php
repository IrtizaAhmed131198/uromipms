...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $login_background_image = System::getProperty('login_background_image');

        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types', 'login_background_image'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $existing_login_background = System::getProperty('login_background_image');

            if (! empty($request->input('remove_login_background_image'))) {
                $this->deleteLoginBackgroundImage($existing_login_background);
                System::removeProperty('login_background_image');
                $existing_login_background = null;
            }

            $uploaded_login_background = $this->businessUtil->uploadFile($request, 'login_background_image', 'login_backgrounds', 'image');

            if (! empty($uploaded_login_background)) {
                $this->deleteLoginBackgroundImage($existing_login_background);
                System::updateOrCreate(
                    ['key' => 'login_background_image'],
                    ['value' => $uploaded_login_background]
                );
            }
    ...
    /**
     * Delete login background image from storage
     *
     * @param  string|null  $fileName
     * @return void
     */
    protected function deleteLoginBackgroundImage($fileName = null)
    {
        if (empty($fileName)) {
            return;
        }

        $path = 'login_backgrounds/'.$fileName;
