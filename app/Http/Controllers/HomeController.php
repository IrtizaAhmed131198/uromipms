...
class HomeController extends Controller
{
    ...
    {
        $location_code = explode('-', $slug)[0];

        $location = BusinessLocation::where('location_id', $location_code)
            ->where('is_active', 1)
            ->firstOrFail();

        $categories = Category::where('business_id', $location->business_id)
            ->where('parent_id', 0)
            ->where('category_type', 'product')
            ->where($this->categoryVisibleAtLocationClosure($location))
            ->get();

        return view('locations.location', compact('location', 'categories'));
    }
    ...
    public function category($slug, $category_id, Request $request)
    {
        ...
        $category = Category::where('business_id', $business_id)->findOrFail($category_id);
        ...
        // Active subcategory (NULL if not selected)
        $active_subcategory_id = $request->query('sub');
        ...
        $itemsQuery = Product::query()
            ...
            ->where([
                ...
                'pl.location_id' => $location->id,
                'products.not_for_selling' => 0,
            ]);
                

        // Filter ONLY if subcategory selected
        if ($active_subcategory_id) {
            $itemsQuery->where('products.sub_category_id', $active_subcategory_id);
        }
        ...
        // All items count (for All Items tab)
        $all_items_count = Product::join('product_locations as pl', 'pl.product_id', '=', 'products.id')
            ->where([
                'products.business_id' => $business_id,
                'products.category_id' => $category_id,
                'pl.location_id' => $location->id,
        ...
        foreach ($subcategories as $sub) {
            $sub->products_count = Product::join('product_locations as pl', 'pl.product_id', '=', 'products.id')
                ->where([
                    'products.business_id' => $business_id,
                    'products.category_id' => $category_id,
                    'products.sub_category_id' => $sub->id,
                    'pl.location_id' => $location->id,
                    'products.not_for_selling' => 0,
                ])
                ->count();
        ...
        return view('locations.category', compact(
            'location',
            'category',
            'subcategories',
            'items',
    ...
    public function subcategory($slug, $category_id, $subcategory_id)
    {
        $location_code = explode('-', $slug)[0];
        ...
        $category = Category::where('business_id', $business_id)->findOrFail($category_id);
        $subcategory = Category::where('business_id', $business_id)->findOrFail($subcategory_id);
        ...
        $items = Product::query()
            ...
            ->groupBy('products.id')
            ->get();
        return view('locations.subcategory', compact(
            'location',
            'category',
            'subcategory',
            'items'
    ...
    /**
     * Categories shown on a location menu: assigned to this location, or legacy rows with no restriction.
     *
     * @return \Closure
     */
    protected function categoryVisibleAtLocationClosure(BusinessLocation $location)
    {
        $lid = (int) $location->id;

        return function ($query) use ($lid, $location) {
            $query->whereNull('business_location')
                ->orWhere('business_location', '')
                ->orWhere('business_location', '[]')
                ->orWhere(function ($q) use ($lid) {
                    $q->whereJsonContains('business_location', $lid)
                        ->orWhereJsonContains('business_location', (string) $lid);
                });

            // Stored 0 when the business has only one location — treat as assigned to that location.
            $single = Category::singleBusinessLocationIdsOrEmpty((int) $location->business_id);
            if (count($single) === 1 && (int) $single[0] === $lid) {
                $query->orWhere('business_location', 0)
                    ->orWhere('business_location', '0');
            }
        };
    }
}
