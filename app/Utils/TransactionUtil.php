...
class TransactionUtil extends Util
{
     ...
     * @return array
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $sell_line_ids = null)
    {
        $il = $invoice_layout;
        ...
        if ($il->show_letter_head == 1) {
            $output['letter_head'] = !empty($il->letter_head) &&
                file_exists(public_path('uploads/invoice_logos/' . $il->letter_head))
                    ? asset('uploads/invoice_logos/' . $il->letter_head)
                    : null;
        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            // Build query for sell lines
            $lines_query = $transaction->sell_lines()->whereNull('parent_sell_line_id');

            // If sell_line_ids provided, filter to show only newly added items (for printing)
            if (!empty($sell_line_ids) && is_array($sell_line_ids) && count($sell_line_ids) > 0) {
                $lines_query->whereIn('id', $sell_line_ids);
            }

            $lines = $lines_query->with($sell_line_relations)->get();
        ...
        // If filtering by sell_line_ids (printing only newly added items), calculate totals from filtered lines only
        if (!empty($sell_line_ids) && is_array($sell_line_ids) && count($sell_line_ids) > 0) {
            // Recalculate subtotal from filtered lines only
            $filtered_subtotal = $subtotal_exc_tax + $total_line_taxes;
            $output['subtotal'] = $this->num_f($filtered_subtotal, $show_currency, $business_details);
            $output['subtotal_unformatted'] = $filtered_subtotal;
        } else {
        ...
        // Total
        // When printing only newly added items, calculate total from filtered lines only
        if (!empty($sell_line_ids) && is_array($sell_line_ids) && count($sell_line_ids) > 0) {
            // Calculate total from filtered lines: subtotal + line taxes - line discounts
            $filtered_total = $subtotal_exc_tax + $total_line_taxes - $total_line_discount;
            if ($transaction_type == 'sell_return') {
                $output['total_label'] = $invoice_layout->cn_amount_label . ':';
            } else {
                $output['total_label'] = $invoice_layout->total_label . ':';
            }
            $output['total'] = $this->num_f($filtered_total, $show_currency, $business_details);
            $output['total_unformatted'] = $filtered_total;
        } else {
            if ($transaction_type == 'sell_return') {
                $output['total_label'] = $invoice_layout->cn_amount_label . ':';
        ...
        if (!empty($il->common_settings['show_total_in_words'])) {
            ...
            $total_for_words = !empty($sell_line_ids) && is_array($sell_line_ids) && count($sell_line_ids) > 0
                ? $output['total_unformatted']
                : $transaction->final_total;
            $output['total_in_words'] = $this->numToWord($total_for_words, null, $word_format);
        }
    ...
    public function getInputTax($business_id, $start_date = null, $end_date = null, $location_id = null, $contact_id = null)
    {
        ...
        $transaction_tax_details = $query1
            ->groupBy('T.id')
            ->get();

        $product_tax_details = $query2
            ->groupBy('T.id')
            ->get();
        $tax_details = [];
    ...
    public function getSellsCurrentFy($business_id, $start, $end)
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join) {
            $join
                ->on('SR.return_parent_id', '=', 'transactions.id')
    ...
    public function getTransactionTotals(
        ...
        if (!empty($permitted_locations)) {
            if ($permitted_locations != 'all') {
                ...
                if (in_array('payroll', $transaction_types)) {
                    $query
                        ->leftjoin('users as u1', 'u1.id', '=', 'transactions.expense_for')
        ...
        if (!empty($location_id)) {
            ...
            if (in_array('payroll', $transaction_types)) {
                $query
                    ->leftjoin('users as u', 'u.id', '=', 'transactions.expense_for')
        ...
        if (in_array('purchase_return', $transaction_types)) {
            ...
            $output['total_purchase_return_exc_tax'] =
                !empty($transaction_totals->total_purchase_return_exc_tax)
                    ? $transaction_totals->total_purchase_return_exc_tax
                    : 0;
        }
        ...
        if (in_array('sell_return', $transaction_types)) {
            ...
            $output['total_sell_return_exc_tax'] =
                !empty($transaction_totals->total_sell_return_exc_tax)
                    ? $transaction_totals->total_sell_return_exc_tax
                    : 0;

            $output['total_sell_return_discount'] =
                !empty($transaction_totals->total_sell_return_discount)
                    ? $transaction_totals->total_sell_return_discount
                    : 0;
        }
        ...
        if (in_array('expense', $transaction_types)) {
            ...
            $total_expense_refund = !empty($transaction_totals->total_expense_refund)
                ? $transaction_totals->total_expense_refund
                : 0;
        ...
        if (in_array('payroll', $transaction_types)) {
            $output['total_payroll'] =
                !empty($transaction_totals->total_payroll)
                    ? $transaction_totals->total_payroll
                    : 0;
        }
        ...
        if (in_array('stock_adjustment', $transaction_types)) {
            $output['total_adjustment'] =
                !empty($transaction_totals->total_adjustment)
                    ? $transaction_totals->total_adjustment
                    : 0;

            $output['total_recovered'] =
                !empty($transaction_totals->total_recovered)
                    ? $transaction_totals->total_recovered
                    : 0;
        }
        ...
        if (in_array('purchase', $transaction_types)) {
            $output['total_purchase_discount'] =
                !empty($transaction_totals->total_purchase_discount)
                    ? $transaction_totals->total_purchase_discount
                    : 0;
        }
    ...
    /**
     * Delete all sales (draft / suspended / sell / sales_order)
     *
     * @param  int  $business_id
     * @param  array|null  $filters
     * @return array
     */
    public function deleteAllSales($business_id, $filters = [])
    {
        try {
            // 🔹 Build query
            $query = Transaction::where('business_id', $business_id)
            ...
            $transaction_ids = $query->pluck('id');

            if ($transaction_ids->isEmpty()) {
                return [
                    'success' => false,
                    'msg' => __('lang_v1.no_sale_found'),
                ];
            }

            foreach ($transaction_ids as $transaction_id) {
                $result = $this->deleteSale($business_id, $transaction_id);

                // ❌ If any delete fails → stop immediately
                if (!$result['success']) {
                    return $result;
                }
            }
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $previous_transaction_sums = $this
            ->__transactionQuery($contact_id, $start, null, $location_id)
            ->select(
        ...
        $prev_payments = $this
            ->__paymentQuery($contact_id, $start, null, $location_id)
        ...
        $prev_total_advance_payment = $this
            ->__paymentQuery($contact_id, $start, null, $location_id)
        ...
        $transaction_query = $this
            ->__transactionQuery($contact_id, $start, $end, $location_id)
            ->with(['location'])
            ->select('transactions.*');
        ...
        if ($format == 'format_1' || $format == 'format_3') {
            $payments = $this
                ->__paymentQuery($contact_id, $start, $end, $location_id)
        ...
        $total_excess_advance_payment = $this
            ->__paymentQuery($contact_id, $start, $end, $location_id)
            ->select(
                DB::raw('(transaction_payments.amount - COALESCE((SELECT SUM(amount) from transaction_payments as TP where TP.parent_id = transaction_payments.id), 0)) as amount')
            )
        ...
        $overall_transaction_sums = $this
            ->__transactionQuery($contact_id, null, null, $location_id)
            ->select(
                DB::raw("SUM(IF(type = 'purchase', final_total, 0)) as total_purchase"),
                DB::raw("SUM(IF(type = 'sell' AND status = 'final', final_total, 0)) as total_invoice"),
                DB::raw("SUM(IF(type = 'sell_return', final_total, 0)) as total_sell_return"),
        ...
        $overall_payments = $this
            ->__paymentQuery($contact_id, null, null, $location_id)
            ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 'is_advance')
            ->get();
        ...
        $overall_total_advance_payment = $this
            ->__paymentQuery($contact_id, null, null, $location_id)
            ->select('bl.name as location_name',
                't.type as transaction_type',
                'is_advance',
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        $sales_by_subtype = $sales_by_subtype
            ->select(DB::raw('SUM(total_before_tax) as total_before_tax'), 'sub_type')
