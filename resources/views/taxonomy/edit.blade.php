  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        ...
        <div class="checkbox">
          <label>
            <input type="checkbox" id="select_all_locations"> <strong>Select All</strong>
          </label>
        </div>
        <div class="row">
          @php
            $selected_locations = array_map('intval', array_filter((array) ($category->business_location ?? [])));
          @endphp
          @foreach($business_locations as $id => $name)
            <div class="col-md-6">
              <div class="checkbox">
                <label>
                  {!! Form::checkbox('business_location[]', $id, in_array((int) $id, $selected_locations, true), ['class' => 'location_checkbox']) !!} {{ $name }}
                </label>
              </div>
            </div>
          @endforeach
      ...
      <div class="form-group">
        ...
        @if(!empty($category->icon))
          <p class="help-block">Current: <img src="{{ asset($category->icon) }}" style="height:30px"></p>
        @endif
      </div>
    </div>
