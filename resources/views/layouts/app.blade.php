...
<div id="sync-status" style="display: none; position: fixed; bottom: 20px; right: 20px; z-index: 10000;">
    <div class="alert alert-warning" style="margin: 0; padding: 10px 15px;">
        <i class="fa fa-sync-alt fa-spin"></i>
        <span id="sync-status-text">Syncing...</span>
        <button type="button" class="close" onclick="$('#sync-status').hide()">
            <span>&times;</span>
        </button>
    </div>
</div>
...
<script>
// Show sync status
function showSyncStatus(text, type = 'warning') {
    const $status = $('#sync-status');
    const $alert = $status.find('.alert');
...
window.addEventListener('online', () => {
    showSyncStatus('Back online - syncing...', 'info');
});

window.addEventListener('offline', () => {
    showSyncStatus('Offline mode - changes will sync later', 'warning');
});
</script>
