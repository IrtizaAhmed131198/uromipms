...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (add_via_ajax) {
        ...
        // --- Client-side cache for get_product_row ---
        // Initialize global cache if not present. TTL in ms (default 5 minutes).
        window.posApiCache = window.posApiCache || { map: new Map(), ttl: 300000, storageKey: 'posApiCache_v1' };

        // Try to hydrate cache from localStorage if available
        (function hydrateCache() {
            try {
                if (!window.localStorage) return;
                var raw = localStorage.getItem(window.posApiCache.storageKey);
                if (!raw) return;
                var obj = JSON.parse(raw);
                if (!obj || !obj.items) return;
                obj.items.forEach(function(it) {
                    // ignore expired entries
                    if (it.ts && it.key && it.data) {
                        window.posApiCache.map.set(it.key, { ts: it.ts, data: it.data });
                    }
                });
            } catch (e) {
        ...
        function persistCache() {
            try {
                if (!window.localStorage) return;
                var items = [];
                window.posApiCache.map.forEach(function(v, k) {
                    items.push({ key: k, ts: v.ts, data: v.data });
                });
                localStorage.setItem(window.posApiCache.storageKey, JSON.stringify({ items: items }));
            } catch (e) {
                // ignore storage quota errors
            }
        }

        var requestUrl = '/sells/pos/get_product_row/' + variation_id + '/' + location_id;
        var requestData = {
        ...
        var cacheKey = requestUrl + '?' + $.param(requestData);

        function handleProductRowResult(result) {
            if (result.success) {
        ...
        // Check cache first (in-memory/localStorage)
        var cached = window.posApiCache.map.get(cacheKey);
        if (cached && (Date.now() - cached.ts) < window.posApiCache.ttl) {
            try {
                handleProductRowResult(cached.data);
            } catch (e) {
                // Fallback to network if cached handling fails
                window.posApiCache.map.delete(cacheKey);
            }
            return;
        }

        // If offline, try to use Service Worker Cache Storage as a fallback
        if (!navigator.onLine && 'caches' in window) {
            var fullUrl = requestUrl + '?' + $.param(requestData);
            caches.match(fullUrl).then(function(response) {
                    ...
                    response.json().then(function(data) {
                        try {
                            ...
                            if (data && data.success) {
                                window.posApiCache.map.set(cacheKey, { ts: Date.now(), data: data });
                                persistCache();
                                try { idbPut(cacheKey, data).catch(function() {}); } catch (e) {}
                            }
                        } catch (e) {}
                        handleProductRowResult(data);
                    }).catch(function() {
                        toastr.error('Cached response corrupted.');
                        $('input#search_product').focus().select();
                    });
                } else {
                    // No cache found in CacheStorage, try stale local cache if present or IndexedDB
                    if (cached && cached.data) {
                        handleProductRowResult(cached.data);
                    } else if ('indexedDB' in window) {
                        idbGet(cacheKey).then(function(rec) {
                            if (rec && rec.data) {
                                handleProductRowResult(rec.data);
                            } else {
                                toastr.error('Offline and no cached product data available.');
                                $('input#search_product').focus().select();
                            }
                        }).catch(function() {
                            toastr.error('Offline and no cached product data available.');
                ...
                // caches.match failed, fallback to stale local cache or IndexedDB
                if (cached && cached.data) {
                    handleProductRowResult(cached.data);
                } else if ('indexedDB' in window) {
                    idbGet(cacheKey).then(function(rec) {
        ...
        // Online: Make AJAX request and cache successful responses
        $.ajax({
            method: 'GET',
            url: requestUrl,
            async: false,
            ...
            success: function(result) {
                try {
                    if (result && result.success) {
                        // store in cache
                        window.posApiCache.map.set(cacheKey, { ts: Date.now(), data: result });
                        // persist
                        persistCache();
                        try { idbPut(cacheKey, result).catch(function() {}); } catch (e) {}
                    }
                } catch (e) {}
                handleProductRowResult(result);
            },
            error: function() {
                // try to use stale local cache if available
                if (cached && cached.data) {
                    handleProductRowResult(cached.data);
                } else {
                    // As a last resort, try CacheStorage even when online (network blip)
                    if ('caches' in window) {
                        var fullUrl = requestUrl + '?' + $.param(requestData);
                        caches.match(fullUrl).then(function(response) {
                            if (response) {
                                response.json().then(function(data) {
                                    ...
                                    toastr.error('Could not fetch product row.');
                                    $('input#search_product').focus().select();
                                });
                            } else if ('indexedDB' in window) {
                                idbGet(cacheKey).then(function(rec) {
...
// --- IndexedDB helpers for offline storage of product rows ---
function idbOpen() {
    return new Promise(function(resolve, reject) {
        try {
            var req = indexedDB.open('pos-db', 1);
            req.onupgradeneeded = function(e) {
                var db = e.target.result;
                if (!db.objectStoreNames.contains('product_rows')) {
                    db.createObjectStore('product_rows', { keyPath: 'key' });
                }
            };
...
function idbPut(key, data) {
    return idbOpen().then(function(db) {
        return new Promise(function(resolve, reject) {
            try {
                var tx = db.transaction('product_rows', 'readwrite');
                var store = tx.objectStore('product_rows');
                store.put({ key: key, ts: Date.now(), data: data });
...
function idbGet(key) {
    return idbOpen().then(function(db) {
        return new Promise(function(resolve, reject) {
            try {
                var tx = db.transaction('product_rows', 'readonly');
                var store = tx.objectStore('product_rows');
