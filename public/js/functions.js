...
function __select2(selector) {
    if ($('html').attr('dir') == 'rtl') selector.select2({
        dir: 'rtl'
    });
...
let __online_state = navigator.onLine;

window.addEventListener('online', () => __online_state = true);
window.addEventListener('offline', () => __online_state = false);

function __is_online() {

    // Localhost bypass
    if (document.getElementById('__is_localhost')) {
        return true;
    }

    return __online_state;
}
